              ...
              children: [
                ...
                Container(
                  decoration: boxDecorationWithRoundedCorners(
                      ...
                      border: Border.all(color: ColorUtils.colorPrimary.withValues(alpha:0.5))),
                ...
                Container(
                  decoration: boxDecorationWithRoundedCorners(
                      borderRadius: BorderRadius.circular(defaultRadius), border: Border.all(color: Colors.red), backgroundColor: Colors.red.withValues(alpha:0.2)),
                  padding: .symmetric(horizontal: 4, vertical: 4),
                  child: Icon(
                    Icons.close,
                    color: Colors.red,
            ...
            Column(
              ...
              children: [
                ...
                if (data.reScheduleDateTime != null)
                  Text('${language.note} ${language.rescheduleMsg} ${DateFormat('yyyy-MM-dd').format(DateTime.parse(data.reScheduleDateTime!))} ',
                          style: secondaryTextStyle(color: Colors.red, size: 12))
                      .paddingOnly(top: 4)
              ],
            ),
    ...
    if (orderStatus == ORDER_PENDING) {
      ...
      await updateOrderStatusForAssignedTab(req).then((value) {
        if (value.success == false) {
          ...
          int i = statusList.indexWhere((item) => item == ORDER_PENDING);
      ...
      bool isCheck = await ReceivedScreenOrderScreen(orderData: orderData, isShowPayment: (orderData.paymentId == null || orderData.paymentId == 0) && orderData.paymentCollectFrom == PAYMENT_ON_PICKUP)
          .launch(context, pageRouteAnimation: PageRouteAnimation.SlideBottomTop);
      if (isCheck) {
        getOrderListApiCall();
        int i = statusList.indexWhere((item) => item == ORDER_ARRIVED);
      ...
      if (now.isAfter(startTime)) {
        bool isCheck = await ReceivedScreenOrderScreen(orderData: orderData, isShowPayment: (orderData.paymentId == null || orderData.paymentId == 0) && orderData.paymentCollectFrom == PAYMENT_ON_DELIVERY)
            .launch(context, pageRouteAnimation: PageRouteAnimation.SlideBottomTop);
        if (isCheck) {
