...
void main() async {
  ...
  try {
    ...
    appStore.setUserProfile(
      getStringAsync(USER_PROFILE_PHOTO),
      isInitializing: true,
    );
    FilterAttributeModel? filterData = FilterAttributeModel.fromJson(
      getJSONAsync(FILTER_DATA),
    );
    ...
    int themeModeIndex = getIntAsync(
      THEME_MODE_INDEX,
      defaultValue: -1, // -1 = system
    );

    if (themeModeIndex == -1) {
      appStore.followSystemTheme = true;
    } else {
      appStore.followSystemTheme = false;
      appStore.setDarkMode(themeModeIndex == appThemeMode.themeModeDark);
    }
    initJsonFile();
    oneSignalSettings();
  } catch (e) {
...
class MyAppState extends State<MyApp> {
  ...
  void initTheme() {
    ...
    if (themeModeIndex == appThemeMode.themeModeSystem) {
      ...
      appStore.followSystemTheme = false;
      appStore.isDarkMode = themeModeIndex == appThemeMode.themeModeDark;
    }
  }
    ...
    return Observer(
      builder: (context) {
        return MaterialApp(
          ...
          themeMode: appStore.followSystemTheme
              ? ThemeMode.system
