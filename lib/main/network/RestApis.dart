...
import '../models/PayoutRecord.dart';
import '../models/PaytrPaymentResponse.dart';
import '../models/PolylineDataModel.dart';
...
Future<PayoutRecord> getDriverPayout({
  required int page,
  required int driverId,
  required String? status,
}) async {
  return PayoutRecord.fromJson(await handleResponse(await buildHttpResponse('delivery-man-payout-document-list?delivery_man_id=$driverId&page=$page&status=$status', method: HttpMethod.GET)));
}

Future<LDBaseResponse> sendOtpApi({required String contactNumber, required String otpCode}) async {
  return LDBaseResponse.fromJson(await handleResponse(await buildHttpResponse('send-sms?number=$contactNumber&code=$otpCode', method: HttpMethod.POST)));
}