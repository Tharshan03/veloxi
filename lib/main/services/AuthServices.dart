...
class AuthServices {
  Future<void> updateUserData(UserData user) async {
    userService.updateDocument({'player_id': getStringAsync(PLAYER_ID), 'updatedAt': Timestamp.now()}, user.uid);
  }
                    ...
                    await logInApi(request)
                        .then((res) async {
                          ...
                          await signInWithEmailPassword(context, email: email.validate(), password: password.validate()).then((value) {
                            updateUserStatus({"id": getIntAsync(USER_ID), "uid": getStringAsync(UID)}).then((value) {
  ...
  Future<void> loginFromFirebaseUser(User currentUser, {LoginResponse? loginDetail, String? fullName, String? fName, String? lName}) async {
    ...
    if (await userService.isUserExist(loginDetail!.data!.email)) {
      ///Return user data
      await userService
          .userByEmail(loginDetail.data!.email)
          .then((user) async {
            userModel = user;
  ...
  Future<String?> askUserForEmail() async {
  ...
  Future<void> loginFromFirebase(User currentUser, String loginType, String? accessToken, {String userType = CLIENT}) async {
    ...
    if (currentUser.email == null) {
      enteredEmail = await askUserForEmail();
      if (enteredEmail == null || enteredEmail.isEmpty) {
        toast("Email is required to continue.");
        return;
      }
    }

    String safeEmail = currentUser.email == null ? enteredEmail : currentUser.email!;

    Map req = {
      "email": Encryption.instance.encrypt(safeEmail),
      "username": Encryption.instance.encrypt(safeEmail),
      ...
      if(loginType == LoginTypeApple) "apple_user_identifier": Encryption.instance.encrypt(currentUser.uid),
      if (!currentUser.phoneNumber.isEmptyOrNull) 'contact_number': currentUser.phoneNumber.validate(),
    };

    print("Request Data ${req}");

    await logInApi(req, isSocialLogin: true)
        .then((value) async {
